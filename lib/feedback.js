...
export function evaluatePose(motionId, landmarks, history) {
  ...
  switch (motionId) {
    case 1:
      result = evaluateHELP(landmarks); break;
    case 2:
      result = evaluateJellyfishFloat(landmarks); break;
    case 3:
      result = evaluateSignalForHelp(landmarks, history); break;
    case 4:
      result = evaluateSculling(landmarks, history); break;
    case 5:
      result = evaluateDogPaddle(landmarks, history); break;
    case 6:
      result = evaluateBackFloat(landmarks); break;
    default:
      return {
        checks: [],
        overallScore: 0,
  ...
  // 주요 랜드마크 가시성 체크 (손목이 화면 밖이면 경고)
  const wristIndices = [15, 16]; // 양 손목만 체크
  const outOfFrame = wristIndices.filter(i => {
    const lm = landmarks[i];
    return lm.visibility < 0.2 || lm.x < -0.05 || lm.x > 1.05 || lm.y < -0.05 || lm.y > 1.05;
  });
  if (outOfFrame.length >= 1) {
    result.cameraWarning = "손이 화면 밖입니다. 카메라를 조정해주세요";
  }
...
function evaluateSignalForHelp(landmarks, history) {
  ...
  // phase 감지: 팔올리기 vs 흔들기좌 vs 흔들기우
  let phase = null;
  if (anyArmUp && oneArmUp) {
    // 올린 손목의 x 좌표로 좌/우 판단
    const raisedWrist = leftArmUp ? leftWrist : rightWrist;
    const centerX = shoulderMid.x;
    const offset = raisedWrist.x - centerX; // 양수=오른쪽, 음수=왼쪽 (화면 기준)
    const threshold = shoulderWidth * 0.3;

    if (Math.abs(offset) > threshold) {
      // 중심에서 충분히 벗어남 → 좌 또는 우
      phase = offset < 0 ? "흔들기좌" : "흔들기우";
    } else {
      // 중심 근처 → 팔올리기 (아직 흔들지 않음)
      phase = "팔올리기";
    }
  }
 ...
 * 양팔을 허리~가슴 높이에서 벌리기↔모으기 반복
 * phase: 현재 자세가 "벌리기" | "모으기" | null (전환 중)
 */
function evaluateSculling(landmarks, history) {
  ...
  // 1. 팔 높이 체크 (어깨~엉덩이 사이, 여유있게)
  const leftArmHeight =
    leftWrist.y > shoulderMid.y - shoulderWidth * 0.3 &&
    leftWrist.y < hipMid.y + shoulderWidth * 0.5;
  const rightArmHeight =
    rightWrist.y > shoulderMid.y - shoulderWidth * 0.3 &&
    rightWrist.y < hipMid.y + shoulderWidth * 0.5;
  ...
  // 2. 팔 자세 체크 (대칭 + 팔꿈치 각도 통합)
  const wristYDiff = Math.abs(leftWrist.y - rightWrist.y);
  const armsSymmetric = wristYDiff < shoulderWidth * 0.35;
  const leftElbowAngle = angle3D(leftShoulder, leftElbow, leftWrist);
  const rightElbowAngle = angle3D(rightShoulder, rightElbow, rightWrist);
  const elbowOk = leftElbowAngle >= 70 && leftElbowAngle <= 170
    && rightElbowAngle >= 70 && rightElbowAngle <= 170;
  const armPostureOk = armsSymmetric && elbowOk;

  checks.push({
    name: "팔 자세",
    passed: armPostureOk,
    message: armPostureOk
      ? "팔 자세가 좋습니다"
      : !armsSymmetric
        ? "양 팔 높이를 맞추세요"
        : "팔꿈치를 자연스럽게 굽히세요",
  ...
  // 3. 현재 phase 감지 (벌리기 / 모으기)
  const wristDist = distance2D(leftWrist, rightWrist);
  let phase = null;
  if (armsAtCorrectHeight && armPostureOk) {
    if (wristDist > shoulderWidth * 1.6) {
      phase = "벌리기";
    } else if (wristDist < shoulderWidth * 1.0) {
      phase = "모으기";
    }
  }

  checks.push({
    name: phase === "벌리기" ? "벌리기" : phase === "모으기" ? "모으기" : "동작",
    passed: phase !== null,
    message: phase
      ? `${phase} 자세입니다`
      : "팔을 벌리거나 모아주세요",
...
function evaluateDogPaddle(landmarks, history) {
  ...
  // phase 감지: 오른뻗기/오른당기기/왼뻗기/왼당기기
  let phase = null;
  const rightUp = rightWrist.y < shoulderMid.y - shoulderWidth * 0.1;
  const leftUp = leftWrist.y < shoulderMid.y - shoulderWidth * 0.1;
  const rightDown = rightWrist.y > hipMid.y - shoulderWidth * 0.2;
  const leftDown = leftWrist.y > hipMid.y - shoulderWidth * 0.2;
  if (rightUp && !leftUp) {
    phase = "오른뻗기";
